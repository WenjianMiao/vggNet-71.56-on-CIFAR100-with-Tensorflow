...
def batch_activ_conv(current, in_features, out_features, kernel_size):
  with tf.variable_scope("composite_function", reuse = tf.AUTO_REUSE):
    ...
    current = tf.contrib.layers.batch_norm(current, scale=True, is_training=False, updates_collections=None)
...
def batch_activ_fc(current, in_features, out_features):
...
def run_model(data, label_count, depth):
  ...
  with graph.as_default():
    ...
    l2_loss_m = tf.add_n([tf.nn.l2_loss(var) for var in var_list])
    total_loss_m = l2_loss_m * weight_decay + cross_entropy_m
    #train_step_m = tf.train.MomentumOptimizer(lr, 0.9, use_nesterov=True).minimize(loss = total_loss_m, var_list = var_list)
    correct_prediction_m = tf.equal(tf.argmax(ys_m, 1), tf.argmax(ys, 1))
    accuracy_m = tf.reduce_mean(tf.cast(correct_prediction_m, "float"))
    top5_m = tf.nn.in_top_k(predictions=ys_m, targets=tf.argmax(ys,1), k=5)
    top_5_m = tf.reduce_mean(tf.cast(top5_m, "float"))


    with tf.variable_scope("Conv8", reuse = tf.AUTO_REUSE):
      #current = batch_activ_conv(mid, 256, 512, 3, is_training, keep_prob)
      current = batch_activ_conv(mid, 256, 512, 3)
    with tf.variable_scope("Conv9", reuse = tf.AUTO_REUSE):
      #current = batch_activ_conv(current, 512, 512, 3, is_training, keep_prob)
      current = batch_activ_conv(current, 512, 512, 3)
    with tf.variable_scope("Conv10", reuse = tf.AUTO_REUSE):
      #current = batch_activ_conv(current, 512, 512, 1, is_training, keep_prob)
      current = batch_activ_conv(current, 512, 512, 1)
      current = maxpool2d(current, k=2)
    ...
    logits = tf.nn.softmax(ys_)
    top5_result, indices = tf.nn.top_k(logits, 5)
  ...
  with tf.Session(graph=graph, config = config) as session:
    ...
    saver2.restore(session, "vggNetModifyConv7/augmentation.ckpt-120")


    converter = tf.lite.TFLiteConverter.from_session(session, [xs], [logits])
    tflite_model = converter.convert()
    open("converted_model3.tflite", "wb").write(tflite_model);

    '''
    tf.saved_model.simple_save(session, "./SavedModelConv7",
                               {"image": xs, "label": ys, "learning_rate": lr, "prob": keep_prob, "training": is_training},
                               {"accuracy": accuracy})
    '''
    ...
    for epoch in range(1, 125+1):
      ...
      train_data_shuffled, train_labels = shuffle_images_and_labels(train_data_normalization, data['train_labels'])
      ...
      if epoch == 150 or epoch == 225:
        saver2.save(session, 'vggNetModifyConv7/augmentation.ckpt', global_step = epoch)
      elif epoch % 10 == 0:
        saver2.save(session, 'vggNetModifyConv7/augmentation.ckpt', global_step = epoch)
      if epoch % 25 == 0:
...
def run():
  ...
  train_data, train_labels = generateSpecializedData('train', targetList, 500, 0)
  test_data, test_labels = generateSpecializedData("test", targetList, 500, 0)
