...
def run_model(data, label_count, depth):
  ...
  with graph.as_default():
    ...
    with tf.variable_scope("Conv8", reuse = tf.AUTO_REUSE):
      ...
      current = batch_activ_conv(current, 256, 512, 3)
    ...
    with tf.variable_scope("FC17", reuse = tf.AUTO_REUSE):
      ...
      modify = batch_activ_fc(modify, 512, 4096)
    ...
    with tf.variable_scope("Conv11", reuse = tf.AUTO_REUSE):
      #current = batch_activ_conv(current, 512, 512, 3, is_training, keep_prob)
      current = batch_activ_conv(mid, 512, 512, 3)
  ...
  with tf.Session(graph=graph, config = config) as session:
    ...
    converter = tf.lite.TFLiteConverter.from_session(session, [mid], [logits])
